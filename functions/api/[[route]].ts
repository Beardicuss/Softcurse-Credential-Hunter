...
import { getAllProviderStats, getAuditLogs, getKeysByProvider } from '../../server/db';
...
app.get('/hunter/status', async (c) => {
  ...
  return c.json({
    ...
    validKeys: stats.reduce((total, item) => total + Number(item.validKeyCount || 0), 0),
  });
});
...
async function loadProviderKeys() {
  const stats = await getAllProviderStats();
  const providers = Array.from(new Set(stats.map((item) => String(item.provider)).filter(Boolean)));
  return Promise.all(providers.map(async (provider) => ({ provider, keys: await getKeysByProvider(provider) })));
}
